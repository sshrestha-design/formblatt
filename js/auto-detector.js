...
function scanTextLayout(rawBlocks, viewport, pageNum) {
    ...
    for (let line of lines) {
        ...
        // Skip top margin areas (top 8% of page) unless there is an explicit colon or underline
        if (line.y < (viewport.height * 0.08) && !line.str.includes(":") && !/[_]{3,}/.test(line.str)) continue;
...
function fuseDetections(acroFormFields, vectorElements, textResult, rawBlocks, viewport, pageNum, usedNames) {
    ...
    // 2. Add text-based detections ONLY if no existing field covers that row
